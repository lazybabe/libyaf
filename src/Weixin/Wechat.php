...
class Wechat
{
    ...
    const EVENT_MENU_VIEW = 'VIEW'; // 菜单
                                    // -
                                    // 点击菜单跳转链接
    const EVENT_MENU_CLICK = 'CLICK'; // 菜单
                                      // -
                                      // 点击菜单拉取消息
    const EVENT_MENU_SCAN_PUSH = 'scancode_push'; // 菜单
                                                  // -
                                                  // 扫码推事件(客户端跳URL)
    const EVENT_MENU_SCAN_WAITMSG = 'scancode_waitmsg'; // 菜单
                                                        // -
                                                        // 扫码推事件(客户端不跳URL)
    const EVENT_MENU_PIC_SYS = 'pic_sysphoto'; // 菜单
                                               // -
                                               // 弹出系统拍照发图
    const EVENT_MENU_PIC_PHOTO = 'pic_photo_or_album'; // 菜单
                                                       // -
                                                       // 弹出拍照或者相册发图
    const EVENT_MENU_PIC_WEIXIN = 'pic_weixin'; // 菜单
                                                // -
                                                // 弹出微信相册发图器
    const EVENT_MENU_LOCATION = 'location_select'; // 菜单
                                                   // -
                                                   // 弹出地理位置选择器
    const EVENT_SEND_MASS = 'MASSSENDJOBFINISH'; // 发送结果
                                                 // -
                                                 // 高级群发完成
    const EVENT_SEND_TEMPLATE = 'TEMPLATESENDJOBFINISH'; // 发送结果
                                                         // -
                                                         // 模板消息发送结果
    const EVENT_KF_SEESION_CREATE = 'kfcreatesession'; // 多客服
                                                       // -
                                                       // 接入会话
    const EVENT_KF_SEESION_CLOSE = 'kfclosesession'; // 多客服
                                                     // -
                                                     // 关闭会话
    const EVENT_KF_SEESION_SWITCH = 'kfswitchsession'; // 多客服
                                                       // -
                                                       // 转接会话
    const EVENT_CARD_PASS = 'card_pass_check'; // 卡券
                                               // -
                                               // 审核通过
    const EVENT_CARD_NOTPASS = 'card_not_pass_check'; // 卡券
                                                      // -
                                                      // 审核未通过
    const EVENT_CARD_USER_GET = 'user_get_card'; // 卡券
                                                 // -
                                                 // 用户领取卡券
    const EVENT_CARD_USER_DEL = 'user_del_card'; // 卡券
                                                 // -
                                                 // 用户删除卡券
     ...
     * MASSSENDJOBFINISH
     * 或
     * TEMPLATESENDJOBFINISH，即高级群发/模板消息
     */
    public function getRevResult()
    {
        if (isset($this->_receive['Status'])) // 发送是否成功，具体的返回值请参考
                                                  // 高级群发/模板消息
                                                  // 的事件推送说明
    ...
    {
        if (isset($this->_receive['CardId'])) // 卡券
                                                  // ID
            $array['CardId'] = $this->_receive['CardId'];
        if (isset($this->_receive['IsGiveByFriend'])) // 是否为转赠，1
                                                          // 代表是，0
                                                          // 代表否。
            $array['IsGiveByFriend'] = $this->_receive['IsGiveByFriend'];
        if (isset($this->_receive['UserCardCode']) && ! empty($this->_receive['UserCardCode'])) // code
                                                                                                        // 序列号。自定义
                                                                                                        // code
                                                                                                        // 及非自定义
                                                                                                        // code的卡券被领取后都支持事件推送。
     ...
     * 设置回复消息
     * Example:
     * $obj->video('media_id','title','description')->reply();
     *
     * @param string $mediaid
     */
...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    {
        return Cache::ins()->save($cachename, $value, $expired);
    }
    ...
    {
        return Cache::ins()->fetch($cachename);
    }
    ...
    static function json_encode($arr)
    {
        ...
        if (($keys[0] === 0) && ($keys[$max_length] === $max_length)) { // See
                                                                          // if
                                                                          // the
                                                                          // first
                                                                          // key
    ...
    public function uploadForeverMedia($data, $type, $is_video = false, $video_info = array())
    {
        if (! $this->access_token && ! $this->checkAuth())
            return false;
            // #TODO
            // 暂不确定此接口是否需要让视频文件走http协议
            // 如果要获取的素材是视频文件时，不能使用https协议，必须更换成http协议
            // $url_prefix
            // =
            // $is_video?str_replace('https','http',self::API_URL_PREFIX):self::API_URL_PREFIX;
            // 当上传视频文件时，附加视频文件信息
        if ($is_video)
     ...
     *            "filter"=>array(
     *            "is_to_all"=>False,
     *            //是否群发给所有用户.True不用分组id，False需填写分组id
     *            "group_id"=>"2"
     *            //群发的分组id
     *            ),
     *            "msgtype"=>"mpvideo",
     ...
     *         注意：unionid字段
     *
     *         只有在用户将公众号绑定到微信开放平台账号后，才会出现。建议调用前用isset()检测一下
     */
    ...
    /**
     * 删除卡券
     * 允许商户删除任意一类卡券。删除卡券后，该卡券对应已生成的领取用二维码、添加到卡包
     * JS
     * API
     * 均会失效。
     ...
     * 若激活率小于50%，不能新增设备。单次新增设备超过500
     * 个，需走人工审核流程。
     * 审核通过后，可用迒回的批次ID
     * 用“查询设备列表”接口拉取本次申请的设备ID]
     *
     * @param array $data
     *            array(
     *            "quantity"
     ...
     * @param array $data
     *            {"media":'@Path\filename.jpg'}
     *            格式限定为：jpg,jpeg,png,gif，图片大小建议120px*120
     *            px，限制不超过200
     *            px
     *            *200
     *            px，图片需为正方形。
    ...
    /**
     * [addShakeAroundPage
     * 增加摇一摇出来的页面信息，包括在摇一摇页面出现的主标题、副标题、图片和点击进去的超链接。]
     *
     * @param string $title
     *            在摇一摇页面展示的主标题，不超过6
     *            个字
     ...
     * 获取设备信息，包括UUID、major、minor，以及距离、openID
     * 等信息。]
     *
     * @param string $ticket
     *            摇周边业务的ticket，可在摇到的URL
     *            中得到，ticket生效时间为30
     *            分钟
     * @return boolean mixed
     *
